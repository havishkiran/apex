...
export function fmtNavDist(mi, units) {
  const km = units === 'km';
  if (km) {
    const m = mi * 1609.34;
...
export function maneuverVerb(dir) {
  return {
...
export function buildNavMetrics(nav, units) {
  const km = units === 'km';
  const rem = km ? nav.remaining * 1.60934 : nav.remaining;
  const mins = Math.max(1, Math.round(nav.remaining / 28 * 60));
  return {
...
export function TurnBanner({ nav, units, accent, onExit }) {
  const step = nav.step;
  const next = nav.next;
  const arrive = step.dir === 'arrive';
  const d = fmtNavDist(nav.distToTurn, units);
  const imminent = !arrive && nav.distToTurn * 5280 < 600;
    ...
    <div style={{ borderRadius: 22, overflow: 'hidden',
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 15, padding: '14px 12px 14px 14px' }}>
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          {arrive ? (
            <div style={{ fontFamily: COND_NV, fontWeight: 700, fontSize: 30, lineHeight: 1, color: AX.text }}>Arriving</div>
          ) : (
        ...
        <button onClick={onExit} aria-label="Exit route" style={{ width: 34, height: 34, borderRadius: 17,
          ...
          <svg width="14" height="14" viewBox="0 0 14 14">
            <path d="M2 2l10 10M12 2L2 12" stroke={AX.dim} strokeWidth="2" strokeLinecap="round" />
          </svg>
        </button>
      </div>
