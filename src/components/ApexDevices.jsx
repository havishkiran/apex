...
function StatusIndicator({ status, level, battery }) {
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
        {battery != null && (
          <span style={{ display: 'flex', alignItems: 'center', gap: 5 }}>
            ...
            <span style={{ fontFamily: COND, fontSize: 16, fontWeight: 600, color: AX.dim,
  ...
  return (
    <span style={{ display: 'inline-flex', alignItems: 'center', height: 32, padding: '0 14px',
      ...
      fontFamily: SAIRA, fontSize: 12.5, fontWeight: 600, color: AX.dim }}>Connect</span>
  );
}
    ...
    <div onClick={clickable ? () => onSelect(dev.id) : undefined} style={{
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          <span style={{ fontFamily: SAIRA, fontSize: 15.5, fontWeight: 600, color: s.name,
        ...
        <div style={{ fontFamily: SAIRA, fontSize: 12, color: AX.faint, marginTop: 1 }}>{dev.kind}</div>
        <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginTop: 6 }}>
          <span style={{ width: 6, height: 6, borderRadius: 3,
            background: dev.status === 'live' ? AX.orange : dev.status === 'off' ? AX.ghost : AX.dim }} />
          <span style={{ fontFamily: SAIRA, fontSize: 12, fontWeight: 600, letterSpacing: 0.4,
...
export function ApexDevices({ t, onNavigate, gpsSource, onSetSource }) {
  ...
  return (
    <Screen tab="devices" onNavigate={onNavigate}>
      ...
      <p style={{ fontFamily: SAIRA, fontSize: 12, color: AX.faint, textAlign: 'center',
