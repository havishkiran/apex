...
export const EMERGENCY = { name: 'Sam Rivera', rel: 'Emergency contact', phone: '+1 (415) 555‑0142' };

export function CrashAlert({ onDismiss }) {
  const TOTAL = 15;
  const [phase, setPhase] = useState('counting');
  const [left, setLeft] = useState(TOTAL);
  const t0 = useRef(performance.now());
  const raf = useRef(0);
  const phaseRef = useRef('counting');
...
export function VoiceCue({ text, accent }) {
  if (!text) return null;
  return (
