...
function drawCity(canvas, w, h) {
  ...
  for (let gx = -pad; gx < w + pad; gx += step) {
    ...
    let x = gx + (rnd() - 0.5) * 14;
    ctx.moveTo(x, -pad);
    for (let y = -pad; y < h + pad; y += 40) { x += (rnd() - 0.5) * 6; ctx.lineTo(x, y); }
    ctx.stroke();
  }
  for (let gy = -pad; gy < h + pad; gy += step) {
    ...
    let y = gy + (rnd() - 0.5) * 14;
    ctx.moveTo(-pad, y);
    for (let x = -pad; x < w + pad; x += 40) { y += (rnd() - 0.5) * 6; ctx.lineTo(x, y); }
    ctx.stroke();
  }
...
export function ApexMap({ recording = true, accent = '#FF6B1A', following = false, turnDir = 'right' }) {
    ...
    <div ref={wrapRef} style={{ position: 'absolute', inset: 0, overflow: 'hidden', background: '#0E1014' }}>
      ...
      <svg viewBox="0 0 402 874" preserveAspectRatio="xMidYMid slice"
        ...
        {following && (() => {
          ...
          return (
            <g>
              <path d={behind} fill="none" stroke={accent} strokeOpacity="0.28" strokeWidth="5" strokeLinecap="round" />
              ...
              <path d={ahead} fill="none" stroke={accent} strokeWidth="6" strokeLinecap="round" strokeLinejoin="round" />
              {turnDir === 'arrive' && (
