// apex-track.jsx — the Track (home) screen. Recording ⇄ idle.
// Composition: full-bleed dark map breathes up top with the live orange route;
// a bottom glass "instrument console" pairs the big glanceable hero metric with
// the one large record control, secondary readouts beneath. Top overlay carries
// the GPS-source + signal indicator. Tab bar sets the nav language for the suite.

const { useState: useStateT, useEffect: useEffectT, useRef: useRefT } = React;

// colors + shared chrome (Glyph, Bars, TabBar) come from apex-shared.jsx
const C = window.AX;
...
function GpsChip({ source, level, battery, recording }) {
  ...
  return (
    <div style={{
      ...
      <span style={{
        fontFamily: "'Saira', sans-serif", fontSize: 11.5, fontWeight: 600,
        letterSpacing: 0.7, textTransform: 'uppercase', color: C.dim,
      }}>{external ? 'External' : 'Phone'}</span>
...
function LockedPill() {
  return (
    <div style={{
      ...
      <span style={{
        fontFamily: "'Saira', sans-serif", fontSize: 11.5, fontWeight: 700,
        letterSpacing: 1.4, textTransform: 'uppercase', color: C.orange,
      }}>Riding</span>
    </div>
  );
}
...
// ── Safety / lean components ─────────────────────────────────────
// Live lean-angle gauge — orange needle = current lean (the live value).
...
// press-and-hold (850ms) to deliberately unlock while moving
function HoldToUnlock({ onUnlock, accent }) {
  const [p, setP] = useStateT(0);
  const raf = useRefT(0), to = useRefT(0), t0 = useRefT(0), done = useRefT(false);
  const HOLD = 850;
  ...
  const begin = (e) => {
    ...
    // setTimeout guarantees the unlock fires even if rAF is throttled;
    // rAF just drives the smooth ring fill.
...
// the glance-only riding view shown while moving over the safety threshold
...
function Console({ st, t, recording, onToggle, nav }) {
  ...
  const hero = m[t.hero];
  // while following a route, the secondary row becomes nav readouts
  const secondary = nav
  ...
  return (
    <div style={{
      ...
      {/* hero metric + record control share the top row */}
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <div style={{ display: 'flex', alignItems: 'baseline', gap: 10, opacity: idleDim, transition: 'opacity .25s' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 3, marginBottom: 6 }}>
            {hero.unit && <span style={{
              fontFamily: "'Saira', sans-serif", fontSize: 17, fontWeight: 600,
              letterSpacing: 1, textTransform: 'uppercase', color: C.dim,
            }}>{hero.unit}</span>}
            <span style={{
              fontFamily: "'Saira', sans-serif", fontSize: 12, fontWeight: 600,
              letterSpacing: 1.6, textTransform: 'uppercase', color: C.faint,
            }}>{hero.label}</span>
          </div>
        </div>
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 6 }}>
          ...
          <span style={{
            ...
            color: recording ? C.dim : C.orange,
          }}>{recording ? 'Stop' : 'Start'}</span>
        </div>
      </div>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', opacity: nav ? 1 : idleDim, transition: 'opacity .25s' }}>
        {secondary.map((s, i) => (
          <div key={i} style={{ display: 'flex', flexDirection: 'column', gap: 5, flex: 1 }}>
            <span style={{
              fontFamily: "'Saira', sans-serif", fontSize: 11, fontWeight: 600,
              letterSpacing: 1.4, textTransform: 'uppercase', color: C.faint,
            }}>{s.label}</span>
            <span style={{ display: 'flex', alignItems: 'baseline', gap: 5 }}>
              <span style={{
                fontFamily: "var(--apex-num, 'Saira Condensed'), sans-serif", fontWeight: 600, fontSize: 30,
...
function ApexTrack({ st, t, recording, onToggle, onNavigate, cue, nav, onExitRoute }) {
  const [manualUnlock, setManualUnlock] = useStateT(false);
  ...
  useEffectT(() => { if (!shouldLock) setManualUnlock(false); }, [shouldLock]);
  ...
  return (
    <div style={{ position: 'absolute', inset: 0, display: 'flex', flexDirection: 'column' }}>
      ...
      {/* dim scrim under the locked riding view for contrast */}
      ...
      <style>{`
        @keyframes apexBlink { 0%,100%{opacity:1} 50%{opacity:0.25} }
...
Object.assign(window, { ApexTrack });
