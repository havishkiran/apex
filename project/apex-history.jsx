// apex-history.jsx — History (ride list + lifetime summary) and the
// Ride Detail drill-in (full route + restrained speed / elevation / lean charts).
// Uses AX + shared chrome. Orange appears only on the route line (a brand element)
// and on the live "active trace" of a chart — never as decoration.

const { useState: useStateH } = React;

// deterministic noise so a given ride always renders the same series
...
const SAIRA_H = "'Saira', system-ui, sans-serif";
const COND_H = "var(--apex-num, 'Saira Condensed'), sans-serif";
...
function PrTag() {
  return (
    <span style={{ display: 'inline-flex', alignItems: 'center', gap: 4, height: 19, padding: '0 7px',
      ...
      <span style={{ fontFamily: SAIRA_H, fontSize: 9.5, fontWeight: 700, letterSpacing: 1,
    ...
    <button onClick={() => onOpen(ride)} style={{
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 3 }}>
          <span style={{ fontFamily: SAIRA_H, fontSize: 16, fontWeight: 600, color: AX.text,
            whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{ride.name}</span>
          {ride.pr && <PrTag />}
        </div>
        <div style={{ fontFamily: SAIRA_H, fontSize: 12, color: AX.faint, marginBottom: 8 }}>{ride.date}</div>
        <div style={{ display: 'flex', gap: 16 }}>
          {[[dist, km ? 'km' : 'mi'], [ride.dur, ''], [top, km ? 'km/h' : 'mph']].map((m, i) => (
            <span key={i} style={{ display: 'flex', alignItems: 'baseline', gap: 3 }}>
              <span style={{ fontFamily: COND_H, fontWeight: 600, fontSize: 19, color: AX.text,
                fontVariantNumeric: 'tabular-nums' }}>{m[0]}</span>
              {m[1] && <span style={{ fontFamily: SAIRA_H, fontSize: 10.5, fontWeight: 600,
...
function ApexHistory({ t, onNavigate, onOpen }) {
  const km = t.units === 'km';
  ...
  return (
    <div style={{ position: 'relative', height: 188, borderRadius: 20, overflow: 'hidden',
      ...
      <svg viewBox="0 0 362 188" preserveAspectRatio="xMidYMid slice"
        ...
        {/* start (hollow) + end (solid) markers */}
      ...
      <div style={{ position: 'absolute', left: 12, bottom: 12, display: 'flex', gap: 7 }}>
        {[['Start', '8:42'], ['End', '9:25']].map((m, i) => (
          <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 6, height: 26, padding: '0 10px',
            ...
            <span style={{ fontFamily: SAIRA_H, fontSize: 10, fontWeight: 600, letterSpacing: 1,
              textTransform: 'uppercase', color: AX.faint }}>{m[0]}</span>
            <span style={{ fontFamily: COND_H, fontSize: 14, fontWeight: 600, color: AX.text }}>{m[1]}</span>
          </div>
        ))}
      </div>
    </div>
...
// restrained instrument chart — thin trace, optional soft fill, peak dot,
// min/max only. No gridline soup.
    ...
    <div style={{ padding: '15px 16px 12px', background: AX.surface, border: `1px solid ${AX.border2}`,
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 12 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          ...
          <span style={{ fontFamily: SAIRA_H, fontSize: 12, fontWeight: 600, letterSpacing: 1.2,
            textTransform: 'uppercase', color: AX.dim }}>{title}</span>
        </div>
        <span style={{ display: 'flex', alignItems: 'baseline', gap: 4 }}>
          <span style={{ fontFamily: SAIRA_H, fontSize: 10, fontWeight: 600, letterSpacing: 0.6,
            textTransform: 'uppercase', color: AX.faint }}>{peakLabel}</span>
          <span style={{ fontFamily: COND_H, fontSize: 20, fontWeight: 600, color: AX.text,
            fontVariantNumeric: 'tabular-nums' }}>{Math.round(max)}</span>
          <span style={{ fontFamily: SAIRA_H, fontSize: 11, fontWeight: 600, color: AX.dim }}>{unit}</span>
        </span>
      </div>
...
// featured max-lean visual — two arcs fanning left & right to peak lean angle.
...
function ApexRideDetail({ ride, t, onBack }) {
  const km = t.units === 'km';
  ...
  const lean = makeSeries(ride.seed + 200, 40, 8, ride.lean, 0.6);
      ...
      <div className="apex-scroll" style={{ flex: 1, overflowY: 'auto', overflowX: 'hidden' }}>
        <div style={{ padding: '54px 20px 10px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <button onClick={onBack} style={{ display: 'flex', alignItems: 'center', gap: 4, background: 'none',
            ...
            <span style={{ fontFamily: SAIRA_H, fontSize: 15, fontWeight: 500 }}>History</span>
          </button>
          <HeaderIconBtn name="share" />
        </div>
        <div style={{ padding: '0 20px 18px' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 9, marginBottom: 3, minWidth: 0 }}>
            <h1 style={{ margin: 0, fontFamily: COND_H, fontWeight: 700, fontSize: 30, letterSpacing: -0.3,
              color: AX.text, whiteSpace: 'nowrap', flexShrink: 0 }}>{ride.name}</h1>
            {ride.pr && <span style={{ flexShrink: 0 }}><PrTag /></span>}
          </div>
          <div style={{ display: 'flex', alignItems: 'center', gap: 7, fontFamily: SAIRA_H, fontSize: 13, color: AX.faint }}>
...
Object.assign(window, { ApexHistory, ApexRideDetail, APEX_RIDES: RIDES });
